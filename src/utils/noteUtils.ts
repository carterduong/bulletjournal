...
export function splitNote(value: string): string[] {
  return value === "" ? [""] : value.split("\n");
}

export function noteToDocument(value: string): JSONContent {
  return {
    type: "doc",
...
export function documentToNote(document: JSONContent): string {
...
export function getNextDailyKey(
  dayIndex: number,
  noteKeys: string[],
  mondayDate: Date,
): string {
...
export function appendLines(destination: string, lines: string[]): string {
...
export function moveIncompleteLines(
  source: string,
  destination: string,
): MoveResult {
